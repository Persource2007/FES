...
| **View All Stories** | ✅ Yes | ✅ Yes | ❌ No (FES Only) | ❌ No (Own Only) | ✅ Yes (Own Org) | ✅ Yes (Own Org) | ❌ No (Own Only) |
| **Write Stories - Own Org** | ✅ Yes | ✅ Yes | ✅ Yes (FES Only) | ✅ Yes (FES Only) | ✅ Yes | ✅ Yes | ✅ Yes |
| **Publish/Review - All Orgs** | ✅ Yes | ✅ Yes | ❌ No (FES Only) | ❌ No (Submit Only) | ❌ No | ❌ No | ❌ No (Submit Only) |
...
- ✅ Can create/manage categories **for all organizations** (categories are available to all organizations)
...
**Key Distinction:** FES Admin has extensive permissions but cannot create, edit, or manage Super Admin users. Only Super Admin can manage other Super Admin users. FES Admin can create categories that are available to all organizations.
...
**Capabilities:**
- ❌ Cannot manage users
- ❌ Cannot manage categories (can use categories created by Super Admin or FES Admin)
- ✅ Can view stories from **FES organization only** (similar to Org Editor)
- ✅ Can write stories for **FES organization only** in **available categories**
- ✅ Can publish and review stories from **FES organization only**
- ❌ Cannot delete stories
- ✅ Can view activity logs

**Use Case:** FES content editors who review and publish stories within FES organization but don't need administrative privileges. Similar to Organization Editors but for FES resources.
...
- ✅ Can write stories for **FES organization only** in **available categories** (similar to Org Writer)
- ✅ Can submit stories for review (cannot publish directly)
- ❌ Cannot publish/approve stories
- ❌ Cannot delete stories
- ✅ Can view activity logs

**Use Case:** FES content writers who create stories within FES organization but need editorial approval. Similar to Organization Writers but for FES resources.
...
- ❌ Cannot create/manage categories (can use categories created by Super Admin or FES Admin)
...
**Use Case:** Organization administrators who manage their organization's content and users. Can be granted access to multiple organizations by Super Admin or FES Admin for cross-organization management. Categories are created only by Super Admin or FES Admin and are available to all organizations.
...
- ✅ Can write stories for **own organization** in **available categories**
...
| FES Editor | None | None (Use Only) | FES Only | FES Only | FES Only |
| FES Writer | None | None (Use Only) | Own Only | FES Only | Submit Only |
...
|------|----------------|-------------------|--------------|----------------|------------------|----------------|
| Org Admin | Assigned Org(s) Only | None (Use Only) | Assigned Org(s) | Assigned Org(s) | Assigned Org(s) | Assigned Org(s) Only |
| Org Editor | None | None (Use Only) | Own Org | Own Org | Own Org | None |
| Org Writer | None | None (Use Only) | Own Only | Own Org | Submit Only | None |
...
| **Category Management** | Super Admin and FES Admin can create/manage categories for **all organizations** | Org Admin cannot create categories (can use categories created by Super Admin or FES Admin) |
| **Story Access** | FES Editor can view/publish stories from **FES organization only** (similar to Org Editor) | Org Editor can view/publish stories from **own organization only** |
| **Story Creation** | FES Writer can write stories for **FES organization only** (similar to Org Writer) | Org Writer can write stories for **own organization only** |
...
| Org A Admin | Org Admin | ❌ Cannot create | Can only use categories created by Super Admin or FES Admin |
| Org B Admin | Org Admin | ❌ Cannot create | Can only use categories created by Super Admin or FES Admin |
| Org A Editor | Org Editor | ❌ Cannot create | Can only use categories created by Super Admin or FES Admin |
| Org A Writer | Org Writer | ❌ Cannot create | Can only use categories created by Super Admin or FES Admin |

**Result:**
- Org A Editor can write stories in:
  - "Water Conservation" (created by Super Admin)
  - "Forest Rights" (created by FES Admin)
  - ✅ All categories created by Super Admin or FES Admin are available to all organizations
...
- **FES Writer** can write stories and assign them to:
  - ✅ FES organization only
  - ❌ Cannot assign to Org A or Org B
- **FES Writer** can use categories from:
  - ✅ All categories (created by Super Admin or FES Admin)
...
- **Org A Editor** can publish stories from:
  - ✅ Org A organization only
  - ❌ Cannot publish Org B or FES stories
...
| FES Editor | FES Editor | FES Only | FES Only | ❌ No |
...
1. **Org A Writer** writes a story about "Water Conservation in Maharashtra"
   ...
   - Uses "Water Conservation" category (created by Super Admin or FES Admin)
   - Status: Pending Review
2. **Org A Editor** can see and publish this story
3. **FES Editor** cannot see this story (limited to FES resources only)
...
**Result for All Organizations:**
- ✅ Can use "Global Category" (from Super Admin)
- ✅ Can use "FES Category" (from FES Admin)
- ✅ All categories created by Super Admin or FES Admin are available to all organizations
- ❌ Org Admins cannot create categories - they can only use categories created by Super Admin or FES Admin
...
1. **Org A Writer** creates story:
   ...
   - Organization: Org A
   - Category: "Water Conservation" (created by Super Admin or FES Admin)
...
3. **Published Story Visibility:**
   ...
   - ✅ Visible to FES Admin, Super Admin (cross-org access)
   - ❌ Not visible to FES Editor (limited to FES resources only)
...
1. **FES Writer** creates story:
   ...
   - Organization: **FES** (FES Writer limited to FES organization)
   - Category: "Forest Rights" (created by FES Admin)
   - Status: **Draft** → **Submitted for Review**

2. **FES Editor** sees pending story:
   - Story is from FES organization
...
4. **Story Deletion:**
   ...
   - ✅ Super Admin can delete this story (can delete from all orgs)
   - ❌ Org A Admin cannot delete this story (different organization)
...
**Answer:** ❌ No. FES Writer can only write stories for FES organization (similar to Org Writer).
...
**Answer:** ❌ No. FES Editor can only publish stories from FES organization (similar to Org Editor).

### Scenario: Can Org A Admin create a category?
**Answer:** ❌ No. Only Super Admin and FES Admin can create categories. All categories are available to all organizations.
...
### Scenario: Can Org A Writer use a category created by Super Admin or FES Admin?
**Answer:** ✅ Yes. All categories created by Super Admin or FES Admin are available to all organizations.
...
- ⚠️ **Categories:** Categories can only be created by Super Admin or FES Admin. All categories are available to all organizations. Organization Admins, Editors, and Writers can use any category created by Super Admin or FES Admin.
- ⚠️ **Story Deletion:** 
